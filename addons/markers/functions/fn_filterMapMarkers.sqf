...
private _newMapMarkers = allMapMarkers select { !(_x in bwi_markers_filteredMarkers) };
...
// Iterate through all new map markers.
{
	...
	if ( !isNil "_playerSlotIdArr" && _markerId find "BWI_QM" >= 0 ) then {
		...
		private _markerSlotIdArr = [_markerSlotId] call bwi_common_fnc_getSlotIdArray;
	...
	bwi_markers_filteredMarkers pushBackUnique _markerId;
