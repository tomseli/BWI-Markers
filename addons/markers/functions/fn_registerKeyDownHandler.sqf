...
bwi_markers_keyDownEHID = _display displayAddEventHandler["KeyDown",
	{
		...
		{
			_return = [_display] call bwi_markers_fnc_onDeleteDown;
		};
		
		// If the end key was pressed.
		if ( _keyCode == DIK_END ) then 
		{
			_return = [_display] call bwi_markers_fnc_onEndDown;
		};
